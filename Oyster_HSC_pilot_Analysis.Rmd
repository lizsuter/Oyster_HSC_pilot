---
title: "Processing of Results from DADA2/DECIPHER Pipeline of Oyster + HSC temperature experiments"
author: "Liz Suter"
date: "Jan. 13 2021"
---
...
count_table <- read_tsv(file="dada2_results/ASVs_counts.tsv")
...
taxonomy <- read_tsv(file="dada2_results/ASVs_taxonomy.tsv")
# And specify that the first column of data are rownames
...
Many of these seem to be sequenced to completion but for the low ASV abundance ones, it's unclear.

### Remove singletons

```{r}
count_table_no_singletons <- filter(count_table,rowSums(count_table)>1)
# retains all 2439 ASVs
```
...
```{r}
save.image("backups/upto_alignment.RData")
```


Use phangorn package to build tree. Here we are building a maximum likelihood neighbor-joining tree. (Also takes a while to run. Comment out unless running)
...
```{r}
save.image("backups/upto_tree.RData")
```
...
META	=	sample_data(data.frame(metadata, row.names = metadata$`seq_ID`))
...
table(tax_table(ps)[, "phylum"], exclude = NULL)
```
After the above, 1685 ASVs remain
...
```
The HSC samples seem to have more Verrucomicrobiota and less Firmicutes than the oyster samples. Otherwise they look kind of similar across the two animal types 
...
```
That reduced the otu table from 1685 ASVs to 97- note this is definitely not statistically representative. Just peak at some data for now.
